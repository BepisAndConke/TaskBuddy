package nl.fontys.TaskBuddy.configuration.exceptionhandler;
...
public class RestCustomExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleUnauthorizedAccessException(UnauthorizedAccessException error) {
        log.error("UnauthorizedAccessException with status {} occurred.", HttpStatus.UNAUTHORIZED, error);

        List<ValidationErrorDTO> errors = error.getReason() != null ?
                List.of(new ValidationErrorDTO(null, error.getReason()))
                : Collections.emptyList();

        return ResponseEntity
                .status(HttpStatus.UNAUTHORIZED)
                .body(convertToProblemDetail(HttpStatus.UNAUTHORIZED, errors));
    }
